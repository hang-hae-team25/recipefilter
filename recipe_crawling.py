...
for i in range(0, 3):
    ...
    for i in range(0, len(trsImg)):
        ...
        meatSet = {'오리 훈제', '스팸 작은거', '차돌 또는 등심', '소고기(양지)', '돼지고기 (앞다리살)', '베이컨 작은 거', '돈까스', '국거리소고기', '훈제오리',
                   ...
                   '소고기 갈빗살', '소고기 척아이롤', '살치살', '훈제 닭가슴살', '등갈비', '다진 돼지고기', '통삼겹살', '돼지고기앞다리', '갈은 소고기', '스팸 또는 베이컨',
                   '닭가슴살(닭다리살/닭안심)',
                   '돼지 앞다리살', '간돼지고기', '스팸 다진것', '스팸', '불고기용 소고기', '돼지고기앞다리살(제육용)', '삽겹살', '닭가슴살', '닭가슴살 소세지', '쇠고기',
                   '돼지등뼈',
                   '후랑크 소시지', '스팸 작은 거', '리챔', '돼지목살', '치킨 너겟', '소고기국거리', '닭다리', '정육된 닭다리살', '통조림 햄 제일 작은거', '소고기불고기',
                   '볶음탕용 닭한마리',
                   '비엔나소시지 통통한 거', '닭가슴살통조림', '갈은 돼지고기', '꼬꼬빌 닭가슴살', '목살', '닭고기', '돼지고기다짐육', '닭가슴살 통조림', '프랑크소시지',
                   '스팸(마일드)', '베이컨',
                   '닭안심', '스테이크용 소고기등심', '닭(볶음용)', '햄 or 소세지', '비엔나 소세지 10개정도', '소고기 국거리용', '소고기등심', '소고기 다짐육',
                   '분홍 소시지', '다진돼지고기',
                   '닭', '오리훈제 슬라이스', '닭가슴살 통조림(135g)', '베이컨이나 돼지고기 안심', '통조림 닭가슴살', '미니 돈까스', '스팸(小)', '대패 삼겹살',  '햄',
                   '비엔나소세지',  '비엔나소세지 작은거', '차돌박이',}
        ...
        fishSet = {'쭈꾸미', '꽁치캔', '낙지(大)', '손질된 고등어', '숙성 연어', '손질한 새우', '손질된 오징어', '훈제 오리고기', '마일드 참치', '명태살', '생새우',
                   ...
                   '고추참치', '낙지', '참치', '건새우',
                   '크래미 또는 맛살', '자반고등어', '참치캔(100g)', '어묵', '캔참치', '오징어 자른것', '참치캔', '통조림참치캔', '참치캔 (中)', '참치 통조림',
                   '꼬막',
                   '참치(작은캔)', '연어 살', '오징어(小)', '자숙새우', '참치 큰캔', '칵테일새우 또는 자숙새우', '오징어다리', '냉동 새우', '참치 캔', '통조림참치',
                   '손질 오징어', '오징어몸통',
                   '깐새우', '참치캔 150g짜리', '연어', '고등어', '냉동 새우살', '새우', '냉동새우', '칵테일새우',   '황태채',  '오징어', '칵테일 새우', '멸치',  '전복',}
        ...
        carboSet = {'옥수수 전분', '또띠아', '햇감자 주먹만한크기', '참드림쌀 밥', '공기밥', '옥수수 가루', '백설탕', '밥 작은', '자투리 모닝빵', '고구마(작은 것)',
                    ...
                    '찬 밥', '부침가루(밀가루)', '파스타 면', '스파게티면', '소면', '고구마 3개 /큰거', '밀가루(아무거나)', '우동사리', '찬밥', '떡국떡', '떡볶이떡',
                    '8인치 또띠아', '바게트',  '감자', '밥', '고구마', '불린 당면',
                    '국수', '식빵', }
        ...
        milkSet = {'모짜렐라치즈(선택사항)', '흰우유', '플레인요구르트', '크림치즈', '체다 치즈', '체더 슬라이스치즈', '찬 버터', '스틱 모차렐라치즈', '모자렐라 치즈',
                   ...
                   '차가운 버터', '치즈', '요플레', '실온 무염버터', '녹인 무염버터', '무염 버터', '저지방 우유', '플레인 요거트', '실온 우유', '플레인 요구르트', '피자치즈',
                   '모짜렐라 치즈',  '버터', }
        ...
        vegiSet = {'다진홍청양고추', '브로콜리 작은', '빨강파프리카', '페페론치노', '마늘 다진것', '브로컬리', '다진청피망', '완숙토마토', '알배추 작은 거',
                   ...
                   '양파', '작은대파', '채썬 당근', '달래', '채 썬 표고버섯', '배추(노란안쪽만사용)', '로메인', '부추', '아스파라거스', '풋고추', '당근', '청경채',
                   '고추', '파슬리', '가지', '다진 파', '팽이버섯', '간마늘', '송송썰은 대파', '숙주' '시금치', '양상추(샐러드채소)', '무', '양파 작은것', '애호박',
                   '마늘', '파프리카(빨강/노랑) 각', '마늘(편으로썬)', '파프리카(노랑/빨강)', '쪽파or대파', '청 고추 &홍 고추', '알배추', '느타리버섯', '어린잎채소', '작은양파',
                   '노란 파프리카', '파프리카 빨강이', '쌀밥', '새송이버섯', '양배추', '각종자투리 야채', '숙주나물', '다진파',  '쪽파 또는 대파', '콩나물', '다진마늘', '대파 잎', '송송 썬 쪽파',
                   '오이고추', '송이버섯',  '미니새송이버섯',  '데쳐서 물기짠 봄동', '속배추', '표고버섯',  '다진 대파', '다진양파', '다진 쪽파', '파프리카', '배추',
                   '노랑파프리카', '오이', '쪽파',  '대파 다진거', '양송이버섯'}
        if len(trsDetail) != 0:
            i = 1
